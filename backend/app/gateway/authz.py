...
def _resolved_route_signature(func: Callable[..., Any]) -> inspect.Signature:
    """Return a signature with postponed annotations resolved for FastAPI.

    FastAPI inspects the decorator wrapper but resolves annotations against the
    wrapper module. Routes using ``from __future__ import annotations`` would
    otherwise leave request models as unresolved ForwardRef query parameters.
    """
